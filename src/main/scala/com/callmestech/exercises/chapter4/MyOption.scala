...
object MyOption {
  ...
  def sequence[A](xs: List[MyOption[A]]): MyOption[List[A]] = {
    ...
    @tailrec
    def go(xs: List[MyOption[A]], acc: MyOption[List[A]]): MyOption[List[A]] = xs match {
      case Nil => acc
      case ::(head, next) => go(next, head.flatMap(a => acc.map(as => a :: as)))
    }

...
object ParSync {
  ...
  def unit[A](a: A): ParSync[A] = _ =>
    UnitFuture(a)

  def fork[A](a: => ParSync[A]): ParSync[A] = es =>
    es.submit(() => a(es).get())
  ...
  def sequenceBalanced[A](ps: IndexedSeq[ParSync[A]]): ParSync[IndexedSeq[A]] = fork {
  ...
  def parFoldMap[A, B](as: List[A])(z: B)(f: A => B)(op: (B, B) => B): ParSync[B] =
    as.foldRight(unit(z))((a, parB) => fork(map2(lazyUnit(f(a)), parB)(op)))

  /** Implement map3, map4, and map5, in terms of map2.
   */
  def map3[A, B, C, D](parA: ParSync[A],
                       parB: ParSync[B],
                       parC: ParSync[C])
                      (combine: (A, B, C) => D): ParSync[D] =
    map2(
      map2(parA, parB)((a, b) => combine(a, b, _)),
      parC
    )(_ (_))

  def map4[A, B, C, D, E](parA: ParSync[A],
                          parB: ParSync[B],
                          parC: ParSync[C],
                          parD: ParSync[D])
  ...
  def map5[A, B, C, D, E, F](parA: ParSync[A],
                             parB: ParSync[B],
                             parC: ParSync[C],
                             parD: ParSync[D],
                             parE: ParSync[E])
  ...
  def choiceViaChoiceN[A](cond: ParSync[Boolean])
   ...
   *
   * */
  def choiceMap[K, V](key: ParSync[K])(choices: Map[K, ParSync[V]]): ParSync[V] = es =>
    run(es)(map(key)(k => run(es)(choices(k)).get()))

  /** Exercise 7.13
   *
   * Implement this new primitive chooser, and then use it to implement choice and choiceN.
   * */
  def flatMap[A, B](pa: ParSync[A])
                   (choices: A => ParSync[B]): ParSync[B] = es => {
    val a = run(es)(pa).get()
    run(es)(choices(a))
  }

  def choiceViaFlatMap[A](cond: ParSync[Boolean])
                         (onTrue: ParSync[A], onFalse: ParSync[A]): ParSync[A] =
    flatMap(cond)(b => if (b) onTrue else onFalse)

  def choiceNViaFlatMap[A](n: ParSync[Int])(choices: List[ParSync[A]]): ParSync[A] =
   ...
   */
  private case class Map2Future[A, B, C](a: JFuture[A],
                                         b: JFuture[B],
                                         fn: (A, B) => C) extends JFuture[C] {
